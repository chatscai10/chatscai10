...
/**
 * 處理通知請求發送到 LINE 和 Telegram
 * @param {DocumentSnapshot} docSnapshot - 通知請求文檔快照
 * @returns {Promise<Object>} - 處理結果
 */
async function processNotificationRequest(docSnapshot) {
    if (!docSnapshot) {
        logger.error("Invalid document snapshot provided to processNotificationRequest");
    ...
    if (!requestData) {
        logger.error(`No data found in notification request document ${requestId}`);
        return { success: false, error: "No request data found" };
    }

    logger.info(`Processing notification request ${requestId}:`, requestData);
    ...
    try {
        ...
        if (!targetUserIds || targetUserIds.length === 0) {
            logger.warn(`No target users found for request ${requestId}`);
            await docSnapshot.ref.update({ 
                status: 'failed',
        ...
        logger.info(`Found ${targetUserIds.length} target users for request ${requestId}`);

        // 獲取用戶的平台ID (LINE、Telegram)
        const usersWithPlatformIds = await getUsersPlatformIds(
            targetUserIds,
        ...
        let telegramFailure = 0;
        const errors = [];

        // 發送LINE通知
        if (requestData.channels && requestData.channels.includes('line') && 
            usersWithPlatformIds.line && usersWithPlatformIds.line.length > 0) {
            
            const lineClient = getLineClient();
            if (!lineClient) {
                logger.error("LINE client initialization failed");
                errors.push("LINE client initialization failed");
            } else {
                const lineResult = await sendLineNotifications(
                    lineClient,
                    usersWithPlatformIds.line,
                    requestData.title,
                    requestData.body,
                    requestData.link
                );
                
                lineSuccess = lineResult.successCount;
                lineFailure = lineResult.failureCount;
                
                if (lineResult.errors && lineResult.errors.length > 0) {
                    errors.push(`LINE errors: ${lineResult.errors.join('; ')}`);
                }
            }
        }

        // 發送Telegram通知
        if (requestData.channels && requestData.channels.includes('telegram') && 
            usersWithPlatformIds.telegram && usersWithPlatformIds.telegram.length > 0) {
            
            const telegramBot = getTelegramBot();
            if (!telegramBot) {
                logger.error("Telegram bot initialization failed");
                errors.push("Telegram bot initialization failed");
            } else {
                const telegramResult = await sendTelegramNotifications(
                    telegramBot,
                    usersWithPlatformIds.telegram,
                    requestData.title,
                    requestData.body,
                    requestData.link
                );
                
                telegramSuccess = telegramResult.successCount;
                telegramFailure = telegramResult.failureCount;
                
                if (telegramResult.errors && telegramResult.errors.length > 0) {
                    errors.push(`Telegram errors: ${telegramResult.errors.join('; ')}`);
                }
            }
        }

        // 確定最終狀態
        const totalSuccess = lineSuccess + telegramSuccess;
        const totalFailure = lineFailure + telegramFailure;
        const finalStatus = 
            totalSuccess === 0 && totalFailure > 0 ? 'failed' :
            totalFailure > 0 ? 'partial_failure' : 'sent';
        ...
        await docSnapshot.ref.update({
            ...
            completedAt: FieldValue.serverTimestamp(),
            lineSuccessCount: lineSuccess,
            lineFailureCount: lineFailure,
            telegramSuccessCount: telegramSuccess,
            telegramFailureCount: telegramFailure,
            totalSentCount: totalSuccess,
            totalFailureCount: totalFailure,
            error: errors.length > 0 ? errors.join('; ') : null
        });

        logger.info(`Notification request ${requestId} processed. Status: ${finalStatus}, Success: ${totalSuccess}, Failure: ${totalFailure}`);
        ...
        logger.error(`Error processing notification request ${requestId}:`, error);
        ...
        try {
            ...
            logger.error(`Failed to update request ${requestId} status after error:`, updateError);
        }
...
/**
 * 獲取目標用戶IDs
 * @param {string} targetType - 目標類型 ('all', 'store', 'level', 'specific')
 * @param {any} targetValue - 目標值
 * @returns {Promise<string[]>} - 用戶ID陣列
 */
async function getTargetUserIds(targetType, targetValue) {
    let query = db.collection('employees').where('isActive', '==', true);
    ...
    switch (targetType) {
        ...
        case 'store':
            if (!targetValue) {
                logger.warn("Store target specified but no store value provided");
                return [];
            }
            query = query.where('store', '==', targetValue);
            break;
        case 'level':
            if (!targetValue) {
                logger.warn("Level target specified but no level value provided");
                return [];
            }
            query = query.where('roles.level', '==', parseInt(targetValue, 10));
            break;
        case 'specific':
            if (!targetValue || !Array.isArray(targetValue) || targetValue.length === 0) {
                logger.warn("Specific target specified but no user IDs provided");
                return [];
            }
            // 直接返回指定用戶
            return targetValue;
    ...
    if (snapshot.empty) {
        logger.warn(`No employees found matching target: ${targetType} ${targetValue}`);
        return [];
    }
 ...
 * @param {string[]} channels - 需要的平台 ['line', 'telegram']
 * @returns {Promise<Object>} - 包含每個平台用戶ID的物件
 */
async function getUsersPlatformIds(userIds, channels) {
    const result = {
        line: [],
        telegram: []
    };
    ...
    for (let i = 0; i < userIds.length; i += batchSize) {
        ...
        snapshot.forEach(doc => {
            ...
            if (channels.includes('line') && userData.lineUserId) {
                result.line.push({
                    userId: doc.id,
                    platformId: userData.lineUserId,
            ...
            if (channels.includes('telegram') && userData.telegramChatId) {
                result.telegram.push({
                    userId: doc.id,
                    platformId: userData.telegramChatId,
                    name: userData.name || 'Unknown'
                });
            }
        });
...
/**
 * 發送LINE通知
 * @param {line.Client} lineClient - LINE客戶端
 * @param {Object[]} users - 用戶物件陣列，每個物件包含 platformId
 * @param {string} title - 通知標題
 * @param {string} body - 通知內容
 * @param {string} link - 可選的連結
 * @returns {Promise<Object>} - 發送結果
 */
async function sendLineNotifications(lineClient, users, title, body, link) {
    const result = {
        successCount: 0,
        failureCount: 0,
        errors: []
    ...
    if (!lineClient || !users || users.length === 0) {
        return result;
    }
    ...
    for (const user of users) {
        try {
            ...
            // 發送文本消息
            await lineClient.pushMessage(user.platformId, {
                type: 'text',
                text: messageText
            });
            
            result.successCount++;
            logger.info(`LINE notification sent successfully to ${user.name || user.userId} (${user.platformId})`);
        } catch (error) {
            result.failureCount++;
            const errorMsg = `Failed to send LINE notification to ${user.name || user.userId}: ${error.message}`;
...
/**
 * 發送Telegram通知
 * @param {TelegramBot} telegramBot - Telegram機器人
 * @param {Object[]} users - 用戶物件陣列，每個物件包含 platformId
 * @param {string} title - 通知標題
 * @param {string} body - 通知內容
 * @param {string} link - 可選的連結
 * @returns {Promise<Object>} - 發送結果
 */
async function sendTelegramNotifications(telegramBot, users, title, body, link) {
    const result = {
        successCount: 0,
        failureCount: 0,
        errors: []
    ...
    if (!telegramBot || !users || users.length === 0) {
        return result;
    }
    ...
    for (const user of users) {
        try {
            ...
            // 發送消息
            await telegramBot.sendMessage(user.platformId, messageText, options);
            
            result.successCount++;
            logger.info(`Telegram notification sent successfully to ${user.name || user.userId} (${user.platformId})`);
        } catch (error) {
            result.failureCount++;
            const errorMsg = `Failed to send Telegram notification to ${user.name || user.userId}: ${error.message}`;
            result.errors.push(errorMsg);
            logger.error(errorMsg);
        }
    }
...
// --- 發送通知請求處理 Cloud Function ---
exports.sendNotificationRequest = onDocumentCreated(
    {
        document: "notification_requests/{requestId}",
        secrets: [LINE_OA_TOKEN_PARAM, TELEGRAM_BOT_TOKEN_PARAM]
    },
    async (event) => {
        const snapshot = event.data;
        if (!snapshot) {
            logger.error("No document snapshot in sendNotificationRequest event");
            return;
        }
        
        logger.info(`New notification request detected: ${event.params.requestId}`);
        return processNotificationRequest(snapshot);
    }
);

// ... 其他現有的 Cloud Functions ...
 ...
 * 检查用户权限，过滤并返回允许的帖子
 */
exports.getCramSchoolPosts = functions.https.onCall(async (data, context) => {
  // 確保用戶已認證
  if (!context.auth) {
  ...
  console.log(`User ${userId} requesting cram school posts`);
  ...
  try {
    ...
    // 建立基礎查詢 - 所有帖子
    let postsQuery = admin.firestore().collection('cram_school_posts');
    
    // 根據用戶角色添加過濾條件
    if (userRole !== 'admin' && userRole !== 'manager') {
      postsQuery = postsQuery.where('isPublished', '==', true);
      
      // 如果有用戶特定權限，可以添加額外條件
      // 例如: 如果用戶屬於特定分校，只顯示該分校的帖子
      if (userData.branchId) {
        postsQuery = postsQuery.where('branchIds', 'array-contains', userData.branchId);
      }
    }
    
    // 排序 - 首先按照order排序（如果存在），然後按照時間倒序
    postsQuery = postsQuery.orderBy('order', 'asc').orderBy('timestamp', 'desc');
    
    // 執行查詢
    const snapshot = await postsQuery.get();
    
    if (snapshot.empty) {
      console.log('No cram school posts found');
      return { posts: [] };
    }
    ...
    snapshot.forEach(doc => {
      ...
      posts.push({
        ...
        data: {
          ...
          contentHtml: postData.contentHtml || postData.content, // 支持HTML内容
          timestamp: postData.timestamp,
          category: postData.category || '未分類',
          imageUrl: postData.imageUrl || null,
          videoUrl: postData.videoUrl || null,
          order: postData.order || 9999,
          isPublished: postData.isPublished
        }
      });
    });
    
    console.log(`Returning ${posts.length} posts to user ${userId}`);
    return { posts };
    
  } catch (error) {
    console.error('Error in getCramSchoolPosts:', error);
    throw new functions.https.HttpsError(
      'internal',
      '獲取知識+內容時發生錯誤',
      error.message
 ...
 * 安全地验证和处理用户评论提交
 */
exports.addCramSchoolComment = functions.https.onCall(async (data, context) => {
  // 確保用戶已認證
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
  ...
  const userId = context.auth.uid;
  const { postId, commentText, parentId } = data;
  
  // 驗證請求數據
  if (!postId || !commentText || typeof commentText !== 'string') {
    throw new functions.https.HttpsError(
      'invalid-argument',
      '請提供有效的貼文ID和留言內容'
    );
  ...
  if (commentText.trim().length === 0 || commentText.length > 2000) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      '留言內容不能為空且不能超過2000字'
    );
  ...
  try {
    ...
    const userData = userDoc.data();
    const userName = userData.displayName || userData.name || '使用者';
    
    // 检查帖子是否存在
    const postDoc = await admin.firestore().collection('cram_school_posts').doc(postId).get();
    if (!postDoc.exists) {
      throw new functions.https.HttpsError(
        'not-found',
        '找不到指定的貼文'
    ...
    // 检查parentId是否有效（如果提供了）
    if (parentId) {
      const parentComment = await admin.firestore().collection('cram_school_posts').doc(postId).collection('comments').doc(parentId).get();
    ...
    console.log(`Comment added successfully. ID: ${commentRef.id}`);
    
    // 添加帖子评论计数器更新逻辑（可选）
    // const postRef = admin.firestore().collection('cram_school_posts').doc(postId);
    ...
    console.error('Error in addCramSchoolComment:', error);
    throw new functions.https.HttpsError(
      'internal',
      '添加留言時發生錯誤',
      error.message
